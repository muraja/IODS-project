...
1.  The purpose of your analysis is to study the relationships between high/low alcohol consumption and some of the other variables in the data. To do this, choose 4 interesting variables in the data and for each of them, present your personal hypothesis about their relationships with alcohol consumption. **(0-1 point)**
    ...
    I'm choosing the variables sex, age, freetime and address. My hypotheses about their relationships with alcohol consumption are the following:

    -   sex: males have higher use than females

    -   age: I expect alcohol use to peak at 18 when portuguese students can legally buy alcohol

    -   freetime: positive, weak correlation

    -   address: students living in urban areas have higher alcohol used due to easier access to events where alcohol is used.
...
2.  Numerically and graphically explore the distributions of your chosen variables and their relationships with alcohol consumption (use for example cross-tabulations, bar plots and box plots). Comment on your findings and compare the results of your exploration to your previously stated hypotheses. \*\*(0-5 points)
    ...
    alc %>% group_by(address, high_use) %>% summarise(count = n(), age = mean(age))

    # initialize a plot of alc_use and sex
    g1 <- ggplot(alc, aes(x = alc_use, y = address))
...
-   Interpretation of coefficients /OR:
...
4.  *Using the variables which, according to your logistic regression model, had a statistical relationship with high/low alcohol consumption, explore the predictive power of your model. Provide a 2x2 cross tabulation of predictions versus the actual values and optionally display a graphic visualizing both the actual values and the predictions. Compute the total proportion of inaccurately classified individuals (= the training error) and comment on all the results. Compare the performance of the model with performance achieved by some simple guessing strategy. (0-3 points)*
...
select(alc, address, age, freetime, sex, high_use, probability, prediction) %>% tail(10)
...
```{r}
m_all <- glm(high_use ~ address + age + sex + freetime, data = alc, family = "binomial") # add tons of predictors here, then explore, then remove the bad ones
```
